...
$.extend({
  table: function (id, title, headData, field, data) {
    ...
    this.displayBody = function () {
      this.$tbody.empty();
	  //alert("displaybody:"+this.data.length);
	...
	this.replaceImage = function(colName,fillData){
		for (var i = 0; i < this.data.length; i++){
			 this.data[i][colName]="<img src='"+fillData+this.data[i][colName]+"' width='100' height='100'/>";	
		}
	}
